...
#define I2S_NUM 0 // GPIO 25 & GPIO 26
...
void init_microphone(void)
{
	 ...
	 vTaskDelay(5000/portTICK_RATE_MS);	// Delay necessary for reliable start
	 i2s_adc_enable(I2S_NUM);

	 SYSCON.saradc_ctrl2.sar1_inv = 1; // Inversion, to get uninverted values from i2s
}
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    if (eir) {
        ...
        if (strcmp((char *)s_peer_bdname, "Bose NC 700 HP") != 0) {
            return;
        }
...
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
    ...
    size_t bytes_read;
    int16_t d1[len/2];
    i2s_read(I2S_NUM, (void*) d1, len, &bytes_read, portMAX_DELAY);    // read adc data from dma

    int countt = 0;
    int16_t raw = 0;
    for(int i=0; i<len/2; i++){
        raw = (d1[i] & 0xfff)*16 ;    // Scale 12 bit data to 16 bit
        d1[i] = raw;
    }
    memcpy(data, d1, len);
    return len;
